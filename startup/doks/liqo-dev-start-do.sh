...
doctl kubernetes cluster kubeconfig show doks-1 > "$HOME/liqo-kubeconf-do1.yaml"
doctl kubernetes cluster kubeconfig show doks-2 > "$HOME/liqo-kubeconf-do2.yaml"
...
export KUBECONFIG="$HOME/liqo-kubeconf-do1.yaml"
liqoctl install --pod-cidr "$liqo1podcidr" --service-cidr "$liqo1servicecidr" --cluster-name doks-1 &
PIDS+=($!)

export KUBECONFIG="$HOME/liqo-kubeconf-do2.yaml"
